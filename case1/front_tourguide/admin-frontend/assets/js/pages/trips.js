...
document.addEventListener("DOMContentLoaded",function(){
 ...
 state.addEventListener("click",async e=>{
  const view=e.target.closest("[data-trip-view]"),edit=e.target.closest("[data-trip-edit]"),del=e.target.closest("[data-trip-delete]");
  if(view){try{const r=await TL.Trips.getTrips();const rows=P.list(r)||[];const t=rows.find(x=>String(x.id)===String(view.dataset.tripView));document.getElementById("tripDetails").textContent=t?JSON.stringify(t,null,2):"Trip data unavailable.";P.modal("tripDetailsModal")?.show();}catch(err){TL.showToast(err.message,"error");}}
  if(edit){try{const r=await TL.Trips.getTrips();const rows=P.list(r)||[];const t=rows.find(x=>String(x.id)===String(edit.dataset.tripEdit));if(!t){TL.showToast("Trip details unavailable in the current list response.","warning");return;}document.getElementById("trip_id").value=t.id;document.getElementById("trip_num_days").value=t.num_days??"";document.getElementById("trip_travel_style").value=t.travel_style??"";document.getElementById("trip_dis_country").value=t.dis_country??"";document.getElementById("trip_budget").value=t.budget??"";document.getElementById("trip_interst").value=t.interst??"";document.getElementById("trip_number_of_travelers").value=t.number_of_travelers??"";document.getElementById("trip_user_id").value=t.user?.id??t.user_id??"";P.modal("tripEditModal")?.show();}catch(err){TL.showToast(err.message,"error");}}
  if(del&&P.confirm("Delete this trip? This cannot be undone.")){try{await TL.Trips.deleteTrip(del.dataset.tripDelete);TL.showToast("Trip deleted.","success");load();}catch(err){TL.showToast(err.message,"error");}}
 });
