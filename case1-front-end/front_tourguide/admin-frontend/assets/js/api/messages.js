...
(function () {
  ...
  function getContactMessages(query) {
    return window.TL.Api.get("/admin/contact-messages", query);
  }

  // GET /admin/contact-messages/{id}
  function getContactMessage(id) {
    return window.TL.Api.get(`/admin/contact-messages/${encodeURIComponent(id)}`);
  }

  // PUT /admin/contact-messages/{id} — body: { status }
  function updateContactMessage(id, data) {
    return window.TL.Api.put(`/admin/contact-messages/${encodeURIComponent(id)}`, data);
  }

  // DELETE /admin/contact-messages/{id}
  function deleteContactMessage(id) {
    return window.TL.Api.delete(`/admin/contact-messages/${encodeURIComponent(id)}`);
  }
