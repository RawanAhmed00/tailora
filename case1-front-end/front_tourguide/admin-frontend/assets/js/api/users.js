...
(function () {
  ...
  function getUsers(query) {
    return window.TL.Api.get("/admin/users", query);
  }

  // GET /admin/users/{id}
  function getUser(id) {
    return window.TL.Api.get(`/admin/users/${encodeURIComponent(id)}`);
  }

  // POST /admin/users
  // body: { name, email, password, age, dist_country, gender, role, phone_num }
  function createUser(data) {
    return window.TL.Api.post("/admin/users", data);
  }

  // PUT /admin/users/{id}
  // body: { name, email, password, age, dist_country, gender, role, phone_num }
  function updateUser(id, data) {
    return window.TL.Api.put(`/admin/users/${encodeURIComponent(id)}`, data);
  }

  // DELETE /admin/users/{id}
  function deleteUser(id) {
    return window.TL.Api.delete(`/admin/users/${encodeURIComponent(id)}`);
  }

  // PATCH /admin/users/{id}/status — undocumented body shape; pass through as-is
  function changeUserStatus(id, payload) {
    return window.TL.Api.patch(`/admin/users/${encodeURIComponent(id)}/status`, payload);
  }
