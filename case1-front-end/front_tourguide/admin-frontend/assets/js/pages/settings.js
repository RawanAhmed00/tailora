...
document.addEventListener("DOMContentLoaded",function(){
 ...
 function files(){const d={site_name:form.site_name.value,email:form.email.value,phone:form.phone.value,address:form.address.value,social_media_links:form.social_media_links.value};if(form.logo.files[0])d.logo=form.logo.files[0];if(form.homepage_banner.files[0])d.homepage_banner=form.homepage_banner.files[0];Object.keys(d).forEach(k=>{if(d[k]==="")delete d[k];});return d;}
